...
module.exports = function (getJSON) {
  ...
  const WOLEET_API_URL = "https://api.woleet.localhost/v1";
  ...
  woleet.receipt.get = (anchorId) => {
    return getJSON(WOLEET_API_URL + "/receipt/" + anchorId)
      .catch((error) => {
        if (error.code == 404)
  ...
  woleet.identity.getIdentity = (identityUrl, pubKey, leftData) => {
    return getJSON(identityUrl + dataToURI({ pubKey, leftData }))
      .catch((error) => {
        if (error.code == 404)
