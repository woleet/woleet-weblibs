  ...
  function validateIdentity(identityUrl, pubKey) {
    ...
    return getRandomSignature(identityUrl, pubKey, rand)
      .then((res) => {
        ...
        if (/^https?\:\/\/.*/.test(res.rightData) && !res.rightData.startsWith(identityUrl)) {
          return Promise.resolve({valid: false, reason: `rightData parameter begins like an URL (${res.rightData}), therefore it was expected to start with ${identityUrl}`})
        }
