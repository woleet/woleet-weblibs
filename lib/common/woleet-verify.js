  ...
  function validateSignatureAndIdentity(verification) {
    ...
    return Promise.resolve(verification)
      .then((verification) => {
        ...
        // If there is no signature, nothing to do
        ...
        // Validate signature of the message by the public key
        return woleet.signature.validateSignature(message, s.pubKey, s.signature)
          .then((res) => {

            // If signature is invalid, stop
            if (!res.valid) {
              throw new Error('invalid_receipt_signature');
            }

            // If there is no identity URL, nothing to do
            if (!s || !s.identityURL) {
              return verification;
            }
