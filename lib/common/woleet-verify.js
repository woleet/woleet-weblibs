  ...
  function validateSignatureAndIdentity(verification) {
    ...
    return Promise.resolve(verification)
      .then((verification) => {
        ...
        if (s.signedIdentity) {
          message = woleet.crypto.sha256().update(s.signedHash + s.signedIdentity).digest('hex');
        }
        ...
        return woleet.signature.validateSignature(message, s.pubKey, s.signature)
          .then((res) => {
            ...
            // If there is no identity URL
            if (!s.identityURL) {

              // If there is no signed identity, no identity can be returned
              if (!s.signedIdentity) {
                return verification;
              }

              // If the identity is included in the signature, it is verified by the signature verification
              if (s.signedIdentity) {
                verification.identityVerificationStatus = {
                  code: 'verified',
                  identity: woleet.signature.deserializeX500DN(s.signedIdentity)
                };
              }
            }

            // Validate identity of the public key using the identity URL
            return woleet.signature.validateIdentity(s.identityURL, s.pubKey, s.signedIdentity)
              .then((res) => {
                if (!res.valid) {
                  verification.identityVerificationStatus = {
                    code: res.reason,
      ...
      .then((anchorIDs) => {
        ...
        return anchorIDs.reduce((chain, anchorId) => chain
          ...
          .catch((error) => {
            ...
            // If we cannot get the corresponding receipt for this anchorID because
