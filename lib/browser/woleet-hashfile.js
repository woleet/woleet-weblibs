...
/**
 * @returns {String} get the path of the worker script: same as current script's or defined by woleet.workerScriptPath
 */
function findWorkerScriptPath() {
  ...
  if (!workerScriptPath)
    console.error('Cannot find Woleet hashfile worker script ' + DEFAULT_WORKER_SCRIPT);
  else
    console.log('Woleet hashfile worker script will be loaded from: ', workerScriptPath);
...
const isWorkerSupportedPromise = (
  ...
  function checkFileReaderSyncSupport() {
    ...
    const cryptoReachablePromise = isFileReachable(workerScriptPath.split('/').slice(0, -1).join('/')
