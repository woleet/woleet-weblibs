                ...
                return new Promise((next, reject) => {
                    ...
                    worker.onmessage = (event) => {
                        if (event.data.result) {
                            if (cb_result) cb_result({result: event.data.result, file});
                            return next(worker);
                        }
                        else if (event.data.error) {
                            let error = event.data.error;
                            if (cb_error) cb_error({error: new Error(event.data.error), file});
                            else return reject(error);
                        }
                        else if (event.data.start) {
                            if (cb_start) cb_start(event.data);
                        }
                        ...
                        if (stop !== fileSize) {
                            ...
                            if (cb_progress) cb_progress({progress: start / fileSize, file});
                            reader.readAsArrayBuffer(file.slice(start, stop));
                        } else if (stop === fileSize) {
