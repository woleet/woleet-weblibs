...
;(function (factory) {
  ...
  function Hasher() {
        ...
        return new Promise((next, reject) => {
          ...
          worker.onmessage = (event) => {
            if (cancelled) {
              ...
              if (emittable(EVT_ERROR)) emit(EVT_ERROR, {error: new Error(event.data.error), file});
    ...
    function hashLocal(file) {
      ...
      return new Promise((next, reject) => {
        ...
        reader.onprogress = (e) => {
          ...
          emit(EVT_PROGRESS, {progress: e.loaded / e.total, file});

          //noinspection JSUnresolvedVariable
          if (!data)
            return report_update = true;
          //noinspection JSCheckFunctionSignatures,JSUnresolvedVariable
          update(data, e.loaded, e.total);
    ...
    function hashLocalWithNativeAPI(file) {
      ...
      console.log('hashLocalWithNativeAPI');
