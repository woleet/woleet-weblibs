...
;(function (factory) {
    ...
    //noinspection JSUnresolvedVariable
    let workerScriptPath = ( root.woleet && root.workerScriptPath ? root.woleet.workerScriptPath : (basePath ? basePath + DEFAULT_WORKER_SCRIPT : null));
    if (!workerScriptPath)
    ...
    function Hasher() {
        ...
        const HASH_LOCAL_LIMIT = 1e8; // ~100MB
        ...
        const hashLocal = function (file) {
            return new Promise((next, reject) => {
                ...
                reader.onprogress = (e) => {
                    ...
                    //noinspection JSUnresolvedVariable
                    if (!e.target.result && e.loaded === e.total)
